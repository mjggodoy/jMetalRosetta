...
)
{
	...
	if ( scorefxn == nullptr ) {
		...
		std::cout << "XUXA: CREATE SCORE FUNCTION" << std::endl;
...
void DockingLowRes::sync_objects_with_flags()
{
	...
	std::cout << "XUXA: SETUP THE MONTECRLO ALGORITHM" << std::endl;
...
void DockingLowRes::apply( core::pose::Pose & pose )
{
	...
	if ( !flags_and_objects_are_in_sync_ ) {
		...
		std::cout << "XUXA: flags_and_objects_are_in_sync_ " << std::endl;
	...
	if ( first_apply_with_current_setup_ ) {
		...
		std::cout << "XUXA: first_apply_with_current_setup_" << std::endl;
		finalize_setup( pose );
	...
	for ( core::Size i=1; i<=outer_cycles_; ++i ) {
		...
		std::cout << "outer_cycles_: " << i << " " << outer_cycles_ << std::endl;
	...
	std::cout << "SHOWING ENERGY'S POSE'" <<  pose << std::endl;

		//pose.energies().show( std::cout );
	}

	std::cout << "XUXA: VIP LINE: RECUPERAMOS LA ENERGIA MAS PEQUE" <<  pose << std::endl;
...
void DockingLowRes::rigid_body_trial( core::pose::Pose & pose )
{
	...
	std::cout << "XUXA: EVALUANDO???  " << std::endl;
	using namespace moves;
	...
	std::cout << "ROTACION (IMPORTANTE):  " << rot_magnitude_ << std::endl;

	rb_mover_->trans_magnitude( trans_magnitude_ );
	std::cout << "TRANSLACION (IMPORTANTE):  " << trans_magnitude_ << std::endl;
