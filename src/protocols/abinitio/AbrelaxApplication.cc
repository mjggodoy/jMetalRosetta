...
#include <jmetalcpp/metaheuristics/singleObjective/particleSwarmOptimization/PSOSeq.hh>
...
void AbrelaxApplication::add_evaluation( evaluation::PoseEvaluatorOP eval ) {
	//std::cout << "XUXA: ADD EVALUATION" << std::endl;
...
void AbrelaxApplication::setup() {
	...
	if ( option[ in::file::native ].user() ) {
		...
		tr.Info << "XUXA: READ NATIVE POSE IF IT EXISTS " << std::endl;
	...
	core::pose::metrics::PoseMetricCalculatorOP
		clash_calculator( new protocols::toolbox::pose_metric_calculators::ClashCountCalculator( 2.0 ) ); //FUERZAS DE VAN DER WAALs
	...
	io::silent::SilentStruct &pss ) const
{
	...
	std::cout << "XUXA: ESTOY EVALUANDO (AbrelaxApplication::process_decoy)"<< std::endl;
...
void AbrelaxApplication::generate_extended_pose( core::pose::Pose &extended_pose, std::string const& sequence ) const {
	...
	if ( basic::options::option[ basic::options::OptionKeys::in::fix_disulf ].user() ) {
		...
		//std::cout << "XUXA: FIX DISULFIDES" << std::endl;
	...
	for ( Size pos = 1; pos <= extended_pose.size(); pos++ ) {
		...
		//std::cout << "XUXA: MAKE EXTENDED CHAIN" << std::endl;
...
void AbrelaxApplication::setup_fragments() {// FragSetOP& fragsetA, FragSetOP& fragsetB ) const {
	...
	//std::cout << "XUXA: SETUP FRAGMENTS" << std::endl;
...
void AbrelaxApplication::setup_fold( pose::Pose& extended_pose, ProtocolOP& prot_ptr ) {
	...
	if ( option[ OptionKeys::loopfcst::use_general_protocol ] ) {
		...
		if ( option[  OptionKeys::loops::extended_loop_file ].user() ) {
			...
			std::cout << "PUXA: setupfold() HEMOS CREADO UN DoubleLayerKinematicAbinitio" << std::endl;
		} else {
			...
			std::cout << "PUXA: setupfold() HEMOS CREADO UN LoopJumpFoldCst" << std::endl;
		}
		...
		if ( option[  basic::options::OptionKeys::abinitio::membrane ]() ) {
			...
			std::cout << "PUXA: setupfold() HEMOS CREADO UN MembraneAbinitio" << std::endl;
		} else {
			if ( jump_def_ ) {
...
void AbrelaxApplication::fold( core::pose::Pose &init_pose, ProtocolOP prot_ptr ) {
	...
	// determine nstruct
	int const nstruct = std::max( 1, option [ out::nstruct ]() );
	//std::cout << "XUXA: INICIA ABINITIO" << nstruct << std::endl;
	...
	core::scoring::ScoreFunctionOP centroid_scorefxn( generate_scorefxn( false /*fullatom*/ ) );
	core::scoring::ScoreFunctionOP fullatom_scorefxn( generate_scorefxn( true /*fullatom*/ ) );
	//std::cout << "XUXA: fullatom_scorefxn" << fullatom_scorefxn << std::endl;
	abinitio_protocol.set_fullatom_scorefxn( fullatom_scorefxn );
	//std::cout << "XUXA: centroid_scorefxn" << fullatom_scorefxn << std::endl;
	...
	// production loop
	std::cout << "PUXA: EMPIEZAN LOS FOLD JOBS!!!" << std::endl;
	int foldJobCounter = 1;
	while ( jobdist.next_job(curr_job, curr_nstruct) && !bEndrun ) {
		std::cout << "PUXA: FOLD JOB START" << foldJobCounter << std::endl;
		...
		if ( ! abrelax_checkpoints_.recover_checkpoint( fold_pose, jobdist.get_current_output_tag(), "rg_state") ) {
			std::cout << "fold() -> abrelax_checkpoints" << std::endl;
		...
		if ( option[ OptionKeys::abinitio::perturb ].user() ) {
			std::cout << "fold() -> perturb phi/psi randomly" << std::endl;
		...
		jMetal_optimization(prot_ptr, fold_pose);
		...
		std::cout << "PUXA: FOLD JOB END" << foldJobCounter << std::endl;
		foldJobCounter++;
...
void AbrelaxApplication::run() {
	...
	if ( option [ OptionKeys::abinitio::rerun ] ) {// NO ENTRA

		//std::cout << "XUXA: INICIA ABINITIO RERUN" << std::endl;
		do_rerun();
		return;
	}

	if ( option [ OptionKeys::abinitio::jdist_rerun ] ) { //NO ENTRA

		//std::cout << "XUXA: INICIA ABINITIO jdist_rerun" << std::endl;
...
#endif
	std::cout << "XUXA: INICIA ABINITIO setup_fold()" << std::endl;
...
#endif
	std::cout << "XUXA: INICIA ABINITIO fold()" << std::endl;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Maria 8-5-2017 @brief replace application of abinitio protocol in line 1807 that corresponds to the abinitio_protocol.apply( fold_pose ) method;//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
