...
void ClassicAbinitio::apply( pose::Pose & pose ) {
	...
	if ( !bSkipStage1_ ) {
		...
		if ( !get_checkpoints().recover_checkpoint( pose, get_current_tag(), "stage_1", false /* fullatom*/, true /*fold tree */ ) ) {
			...
			tr.Info << "Extracting angles from Pose structure in Stage 1 before do_stage1_cycles: " << std::endl;
			extractAngles(pose);

			success = do_stage1_cycles( pose );
			
			tr.Info << "Extracting angles from Pose structure in Stage 1  after do_stage1_cycles: " << std::endl;
			extractAngles(pose);
	...
	if ( !bSkipStage2_ ) {
		...
		if ( !get_checkpoints().recover_checkpoint( pose, get_current_tag(), "stage_2", false /* fullatom */, true /*fold tree */ ) ) {
			...
			tr.Info << "Extracting angles from Pose structure in Stage 2 before do_stage2_cycles(): " << std::endl;
			extractAngles(pose);

			success = do_stage2_cycles( pose );
			recover_low( pose, STAGE_2 );                    //default OFF: seems to be a bad choice after score0
			
			tr.Info << "Extracting angles from Pose structure in Stage 2 after Stage 1: " << std::endl;
			extractAngles(pose);
			tr.Info << "Extracting angles from Pose structure in Stage 2 after Stage 1 and 2: " << std::endl;
	...
	if ( !bSkipStage3_ ) {
		...
		tr.Info << "Extracting angles from Pose structure in Stage 3: " << std::endl;
		extractAngles(pose);
	...
	if ( !bSkipStage4_ ) {
		...
		tr.Info << "Extracting angles from Pose structure in Stage 4: " << std::endl;
		extractAngles(pose);
	...
	if ( !bSkipStage5_ ) {
		...
		tr.Info << "Extracting angles from Pose structure in Stage 5: " << std::endl;
		extractAngles(pose);
...
bool ClassicAbinitio::do_stage2_cycles( pose::Pose &pose ) {
	...
	if ( apply_large_frags_   ) cycle->add_mover( trial_large_->mover() );//TRUE
	if ( short_insert_region_ ) cycle->add_mover( trial_small_->mover() ); //FALSE

	Size nr_cycles = stage2_cycles() / ( short_insert_region_ ? 2 : 1 ); // ALWAYS 1 (short_insert_region_ = FALSE)
...
void ClassicAbinitio::extractAngles(core::pose::Pose &pose) {
